...
{
    public class RaceScheduleCategory
    {
        const string import_Get = "Import_RaceScheduleCategoryGet";
        const string import_Process = "Import_RaceScheduleCategoryProcess";
        Integrate_Data.DatabaseConnection dbconn;

        public void RaceScheduleCategoryImport(string primaryID, string clubID, string action,string fileNotesID)
        {
            try
            {
                switch (action)
        ...
        private void ProcessDetails(string Index, string Action, DataRow rows = null)
        {
            try
            {
                ...
                if (Action != "Delete")
                {
                    ...
                    dbconn.sqlComm.Parameters.AddWithValue("@RaceScheduleCategoryName", rows["RaceScheduleCategoryName"]);
                    dbconn.sqlComm.Parameters.AddWithValue("@Lap", rows["Lap"]);

...
.controller('MyProfileController', function ($scope, $rootScope, $http, $filter, MainService, ngDialog, ngTableParams) {
    ...
    $scope.UnregMobileNumber = function (mobile, clubID, userid) {
        $scope.MyProfileFilter.MobileNumberSelected = mobile;
        $scope.MyProfileFilter.ClubID = clubID;
        $scope.MyProfileFilter.UserID = userid;
    ...
    $scope.Unreg = function (value) {
        if (value == true) {
            MainService.UnRegMobileNumber($scope.$parent.MyProfileFilter.ClubID, $scope.$parent.MyProfileFilter.MobileNumberSelected, $scope.$parent.MyProfileFilter.UserID).then(function (unregResult) {
                $scope.UnregResult = unregResult;
                alert(unregResult[0].result);
                window.location.href = "/Main/MyProfile";
            }, function ()
...
.factory('MainService', function ($http, $q) {
    ...
    fac.UnRegMobileNumber = function (ClubID, MobileNumber,UserID) {
        var deferred = $q.defer();
        // Initiates the AJAX call
        $http({ method: 'POST', url: '/MyProfile/UnregMobileNumber', params: { MobileNumber: MobileNumber, ClubID: ClubID, UserID: UserID} }).success(deferred.resolve).error(deferred.reject);
        // Returns the promise - Contains result once request completes
        return deferred.promise;
    };
    return fac;

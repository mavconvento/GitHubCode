...
.controller('RaceResultController', function ($scope, $location, $window, $rootScope, $http, $filter, MainService, ngTableParams) {
    ...
    MainService.GetBirdCategory().then(function (BirdCategory) { 
    $scope.BirdCategory = BirdCategory }, function ()
    { alert('error while fetching data from server') })

    MainService.GetGroupCategory().then(function (GroupCategory) { 
    $scope.GroupCategory = GroupCategory }, function ()
    { alert('error while fetching data from server') })
    ...
    $scope.ViewMaps = function (latitude, longtitude,location,coordinates) {
        ...
        $window.open(rooturl + '/main/Maps#?liberationpoint=' + location + '&coordinates=' + coordinates + '&latitude=' + latitude + '&longtitude=' + longtitude, '_blank');
    }
...
.factory('MainService', function ($http, $q) {
    ...
    fac.GetBirdCategory = function () {
        var deferred = $q.defer();
        // Initiates the AJAX call
        $http({ method: 'POST', url: '/RaceResult/GetBirdCategory' }).success(deferred.resolve).error(deferred.reject);
        // Returns the promise - Contains result once request completes
        return deferred.promise;
    };

    fac.GetGroupCategory = function () {
        var deferred = $q.defer();
        // Initiates the AJAX call
        $http({ method: 'POST', url: '/RaceResult/GetGroupCategory' }).success(deferred.resolve).error(deferred.reject);
        // Returns the promise - Contains result once request completes
        return deferred.promise;
    };

...
.controller('RaceResultController', function ($scope, $rootScope, $http, $filter, MainService, ngTableParams) {
    ...
    $scope.SetClubID = function (objects) {
        $scope.ViewLogsFilter.ClubID = objects.clubID;
    };
    ...
    $scope.Submit = function (mobileNumber) {
        ...
        var monthTo = $filter('date')(viewLogsFilterData.DateTo, 'MM'); //December-November like
        var dayTo = $filter('date')(viewLogsFilterData.DateTo, 'dd'); //01-31 like
        var yearTo = $filter('date')(viewLogsFilterData.DateTo, 'yyyy'); //2014 like
...
.factory('MainService', function ($http, $q) {
    ...
    fac.GetLogs = function (d) {
        ...
        $http({ method: 'POST', url: '/ViewLogs/GetViewLogs', params: { ClubID: d.ClubID, MobileNumber: d.MobileNumber, Keyword: d.Keyword, DateFrom: d.DateFrom, DateTo: d.DateTo} }).success(deferred.resolve).error(deferred.reject);

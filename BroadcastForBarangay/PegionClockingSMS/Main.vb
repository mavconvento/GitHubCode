...
#Region "Private Methods"
    ...
    Private Sub InitializeModem()
        ...
        End Try
    End Sub
    Private Sub BroadcastViaConsole()
        Try
            ...
            If mobileNo <> "" And message <> "" Then
                Dim a As Process
                Dim path As String = AppDomain.CurrentDomain.BaseDirectory

                If IsDebug Then
                    path = "C:\Adams\GitHub\GitHubCode\BroadcastForBarangay"
                End If

                If File.Exists(path + "\\SMSSendingQue\\bin\\Debug\\mobileNumber.txt") Then
                    File.Delete(path + "\\SMSSendingQue\\bin\\Debug\\mobileNumber.txt")
                End If

                If File.Exists(path + "\\SMSSendingQue\\bin\\Debug\\message.txt") Then
                    File.Delete(path + "\\SMSSendingQue\\bin\\Debug\\message.txt")
                End If

                Dim pathMobile As String = path + "\\SMSSendingQue\\bin\\Debug\\mobileNumber.txt"
                Dim pathMessage As String = path + "\\SMSSendingQue\\bin\\Debug\\message.txt"

                CreateTextFile(pathMobile, mobileNo)
                CreateTextFile(pathMessage, message)

                a = Process.Start(path + "\\SMSSendingQue\\bin\\Debug\\SMSSendingQue.exe")
                MessageBox.Show("SMS Broadcast Queing is complete.", "Sending Message")
                Me.txtMessage.Text = ""
                Me.txtMobileNumber.Text = ""
            End If
        Catch ex As Exception
        ...
        End Try
    End Sub
    Private Sub CreateTextFile(path As String, content As String)
        Dim fs As FileStream = File.Create(path)
        Dim infoMessage As Byte() = New UTF8Encoding(True).GetBytes(content)
        fs.Write(infoMessage, 0, infoMessage.Length)
        fs.Close()
    End Sub
